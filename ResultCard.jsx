...
export default function ResultCard({
  ...
  const lowConfidence = result.confidence < 65
  const isReal = result.label === 'REAL'
  const tone = lowConfidence ? 'yellow' : isReal ? 'green' : 'red'
  const Icon = lowConfidence ? AlertTriangle : isReal ? CheckCircle2 : ShieldAlert
          ...
          <div>
            ...
            <div className="mt-1 flex flex-wrap items-center gap-2">
              ...
              <span className={`rounded-full px-2.5 py-1 text-xs font-semibold ring-1 ${toneClasses.badge}`}>
                {lowConfidence ? 'Low confidence' : 'Meta-classifier'}
              </span>
            </div>
          </div>
        </div>
