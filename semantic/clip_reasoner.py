...
class CLIPSemanticAnalyzer:
    ...
    def _analyze_with_fallback(self, image: Image.Image) -> dict[str, StreamScore]:
        ...
        nature_render = self._nature_render_proxy(arr, gray)
        architecture_render = self._architecture_render_proxy(arr, gray)
        ...
        nature_strength = normalize_minmax(nature_render["score"], 0.56, 0.86)
        nature_anomaly = clip01(0.56 + 0.34 * nature_strength)
        architecture_strength = normalize_minmax(architecture_render["score"], 0.54, 0.84)
        architecture_anomaly = clip01(0.56 + 0.34 * architecture_strength)
        anomaly = max(
            base_anomaly,
            ...
            nature_anomaly if nature_strength > 0 else 0.0,
            architecture_anomaly if architecture_strength > 0 else 0.0,
        )
        consistency = clip01(1.0 - anomaly)
        details = {
            ...
            "nature_render": nature_render,
            "architecture_render": architecture_render,
        }
        return {
            self.anomaly_name: StreamScore(self.anomaly_name, anomaly, details),
            self.consistency_name: StreamScore(
            ...
            "semantic_portrait_synthetic": StreamScore(
                "semantic_portrait_synthetic",
                portrait_anomaly if portrait_strength > 0 else 0.0,
                portrait_synthetic,
            ),
            "semantic_stylized_composite": StreamScore(
                "semantic_stylized_composite",
                composite_anomaly if composite_strength > 0 else 0.0,
                stylized_composite,
            ),
            "semantic_low_resolution_composite": StreamScore(
                "semantic_low_resolution_composite",
                low_resolution_anomaly if low_resolution_strength > 0 else 0.0,
                low_resolution_composite,
            ),
            "semantic_nature_render": StreamScore(
                "semantic_nature_render",
                nature_anomaly if nature_strength > 0 else 0.0,
                nature_render,
            ),
            "semantic_architecture_render": StreamScore(
    ...
    @staticmethod
    def _nature_render_proxy(arr: np.ndarray, gray: np.ndarray) -> dict[str, float]:
        hue, saturation, value = _rgb_to_hsv(arr)
        green_scene = (
            (hue > 0.16)
            & (hue < 0.46)
        ...
        dark_rock_mass = (value < 0.22) & (saturation < 0.35)
        ...
        edge_band = np.zeros_like(gray, dtype=bool)
        edge_band[:24, :] = True
        edge_band[-24:, :] = True
        edge_band[:, :24] = True
        edge_band[:, -24:] = True

        center_band = np.zeros_like(gray, dtype=bool)
        center_band[56:168, 56:168] = True
        top_center = np.zeros_like(gray, dtype=bool)
        top_center[:80, 70:154] = True

        scene_mass_raw = float(
            np.mean(green_scene | yellow_green | cyan_water_haze | dark_rock_mass)
        )
        green_yellow_mass = float(np.mean(green_scene | yellow_green))
        fine_detail = normalize_minmax(float(np.mean(gradient)), 0.045, 0.105)
        high_detail_tail = normalize_minmax(float(np.percentile(gradient, 90)), 0.12, 0.28)
        dynamic_range = normalize_minmax(
            float(np.percentile(gray, 98) - np.percentile(gray, 10)),
            0.50,
            0.85,
        )
        edge_darkness = float(np.mean(gray[edge_band] < 0.16))
        edge_vignette = normalize_minmax(edge_darkness, 0.25, 0.62)
        sky_haze = float(np.mean((gray[top_center] > 0.55) & (saturation[top_center] < 0.30)))
        bright_haze = normalize_minmax(sky_haze, 0.10, 0.35)
        center_glow = normalize_minmax(float(np.mean(gray[center_band] > 0.62)), 0.03, 0.12)

        scene_mass = normalize_minmax(scene_mass_raw, 0.45, 0.74)
        vegetation_pattern = normalize_minmax(green_yellow_mass, 0.20, 0.45)
        composed_lighting = clip01(
            0.42 * dynamic_range
            + 0.32 * edge_vignette
            + 0.16 * bright_haze
            + 0.10 * center_glow
        ...
        score = clip01(
            0.24 * scene_mass
            + 0.20 * vegetation_pattern
            + 0.22 * fine_detail
            + 0.14 * high_detail_tail
            + 0.20 * composed_lighting
        )
        return {
            "score": score,
            "scene_mass": scene_mass_raw,
            "green_yellow_mass": green_yellow_mass,
            ...
            "sky_haze": sky_haze,
            "bright_haze": bright_haze,
            "center_glow": center_glow,
            "composed_lighting": composed_lighting,
        }

    @staticmethod
    def _architecture_render_proxy(arr: np.ndarray, gray: np.ndarray) -> dict[str, float]:
        hue, saturation, value = _rgb_to_hsv(arr)
        blue_sky = (
            (hue > 0.50)
            & (hue < 0.63)
        ...
        straight_edges = float(
            np.mean(
                (np.maximum(gx, gy) > 0.06)
                & (np.minimum(gx, gy) < 0.50 * np.maximum(gx, gy))
            )
        )
        top = np.s_[:90, :]
        sky_top_mass = float(np.mean(blue_sky[top]))
        sky_grad = (
            float(np.mean(gradient[top][blue_sky[top]]))
            if np.any(blue_sky[top])
            else 1.0
        )
        clean_sky = clip01(
            normalize_minmax(sky_top_mass, 0.30, 0.68)
            * (1.0 - normalize_minmax(sky_grad, 0.018, 0.055))
        )
        lower = np.s_[120:, :]
        plaza_like = float(
            np.mean((value[lower] > 0.20) & (saturation[lower] > 0.03) & (saturation[lower] < 0.35))
        )
        lower_detail = normalize_minmax(float(np.mean(gradient[lower])), 0.045, 0.095)
        sky_brightness = float(np.mean((value[top] > 0.70) & (saturation[top] > 0.35)))

        sky_mass = normalize_minmax(float(np.mean(blue_sky)), 0.16, 0.34)
        stone_mass = normalize_minmax(float(np.mean(warm_stone)), 0.20, 0.42)
        rectilinear = normalize_minmax(straight_edges, 0.12, 0.24)
        plaza_plane = normalize_minmax(plaza_like, 0.18, 0.42)
        saturated_sky = normalize_minmax(sky_brightness, 0.30, 0.70)
        detail_tail = normalize_minmax(float(np.percentile(gradient, 90)), 0.12, 0.25)
        ...
        return {
            "score": score,
            "sky_mass": float(np.mean(blue_sky)),
            "stone_mass": float(np.mean(warm_stone)),
            "straight_edges": straight_edges,
            "sky_top_mass": sky_top_mass,
            "sky_grad": sky_grad,
            "clean_sky": clean_sky,
            "plaza_like": plaza_like,
            "lower_detail": lower_detail,
            "sky_brightness": sky_brightness,
            "rectilinear": rectilinear,
            "plaza_plane": plaza_plane,
