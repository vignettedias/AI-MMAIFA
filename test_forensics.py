...
from forensics import ELAAnalyzer, FrequencyAnalyzer, NoiseAnalyzer, ForensicAnalyzer


def test_forensic_modules_return_normalized_scores(tmp_path):
    image_path = create_demo_image(tmp_path / "real.png", "real")
    image = load_image(image_path)
    for analyzer in [ELAAnalyzer(), FrequencyAnalyzer(), NoiseAnalyzer()]:
...
def test_forensic_ensemble_contains_all_scores(tmp_path):
    ...
    assert {"forensic_ela", "forensic_fft", "forensic_noise", "forensic_ensemble"} <= set(scores)
