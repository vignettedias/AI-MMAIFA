...
def generate_mock_feature_matrix(feature_names: Sequence[str]) -> tuple[np.ndarray, np.ndarray]:
    ...
    for name in [
        ...
        if name in name_to_index:
            ...
            real[:, index] = rng.normal(loc=0.25, scale=0.08, size=24)
            fake[:, index] = rng.normal(loc=0.78, scale=0.10, size=24)
    for name in ["forensic_compression", "expert_social_recompression"]:
        if name in name_to_index:
            index = name_to_index[name]
            real[:, index] = rng.normal(loc=0.32, scale=0.10, size=24)
            fake[:, index] = rng.normal(loc=0.68, scale=0.12, size=24)
